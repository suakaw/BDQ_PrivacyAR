...
class VideoRecord(object):
    def __init__(self, path, start_frame, end_frame, label_actors, label_action, reverse=False):
        self.path = path
...
class VideoDataSet(data.Dataset):
    ...
    def __init__(self, root_path, list_file, num_groups=64, frames_per_group=4, sample_offset=0, num_clips=1,
                 ...
                 image_tmpl='{:08d}.png', transform=None, is_train=True, test_mode=False, seperator=',',
    ...
    def _parse_list(self):
        ...
        for item in tmp:
            ...
            if self.test_mode:
                ...
                '''
                labels_actors.append(float(item[3]))
                labels_action.append(float(item[4])) 
                
                labels_actors = labels_actors[0] if len(labels_actors) == 1 else labels_actors
                labels_action = labels_action[0] if len(labels_action) == 1 else labels_action

                file_list.append([item[0], int(item[1]), int(item[2]), labels_actors, labels_action])
                #print(file_list)
    ...
    def __getitem__(self, index):
        ...
        if self.test_mode:
            ...
            label_action = int(record.label_action)
            label_actors = int(record.label_actors)

        # re-order data to targeted format.
        return images, label_actors, label_action
